...
import { Award, Zap, Clock, ShieldAlert, Play, Pause, RotateCcw, X, Volume2, Film, Trash2 } from 'lucide-react';

export const Leaderboard: React.FC = () => {
  const { leaderboard, resetGame, deleteLeaderboardEntry } = useGameStore();
    ...
    <div className="w-full max-w-4xl text-white font-mono flex flex-col items-center px-4 py-8">
      ...
      <div className="flex flex-wrap gap-2 justify-center mb-6 w-full max-w-2xl bg-white/5 p-1.5 rounded-full border border-white/10">
        {(['all', 'easy', 'medium', 'hard', 'insane'] as const).map((diff) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase transition-all tracking-wider ${filter === diff
      ...
      >
        ...
        <div className="grid grid-cols-12 text-[10px] text-gray-500 pb-3 font-bold tracking-wider px-3 border-b border-white/5">
          ...
          <div className="col-span-3 pl-4">PLAYER</div>
            ...
            filteredEntries.map((entry, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="col-span-3 pl-4 font-bold tracking-widest text-white/90 truncate flex items-center gap-1.5">
                    {entry.playerName}
                    {idx === 0 && <span className="text-[9px] bg-[#D7FF2F]/10 border border-[#D7FF2F]/30 text-[#D7FF2F] px-1 rounded">GOAT</span>}
                  </div>

                  <div className="col-span-2 text-center text-xs">
                    <span className={`px-2 py-0.5 rounded text-[9px] font-bold border uppercase ${entry.difficulty === 'easy' ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400' :
                      entry.difficulty === 'medium' ? 'bg-blue-500/10 border-blue-500/30 text-blue-400' :
                  ...
                  <div className="col-span-2 text-center flex items-center justify-center gap-2">
                    <button
                      ...
                      className="p-1.5 rounded-lg bg-white/5 border border-white/10 text-gray-400 hover:text-[#D7FF2F] hover:border-[#D7FF2F]/40 hover:bg-[#D7FF2F]/5 transition-all cursor-pointer"
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg bg-white/5 border border-white/10 text-gray-400 hover:text-red-500 hover:border-red-500/40 hover:bg-red-500/5 transition-all cursor-pointer"
                      title="Delete Entry"
                    >
          ...
          <div className="flex items-center gap-1.5">
            ...
            {([0.5, 1, 2, 4] as const).map((s) => (
              <button
                ...
                className={`w-8 py-1 rounded text-[10px] font-black tracking-wider transition-all ${speed === s
