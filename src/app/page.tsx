...
import { Play, List, Crosshair, RotateCcw, Clock, Target, Camera, Check, Trash2 } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem('pinchcraft-leaderboard');
      if (stored) {
  ...
  const handleSaveToLeaderboard = (entry: any) => {
    setLeaderboard(prev => {
      ...
      try {
        localStorage.setItem('pinchcraft-leaderboard', JSON.stringify(updated));
      } catch (e) {
        console.error(e);
      }
      return updated;
  ...
  const generatePuzzle = (videoEl: HTMLVideoElement) => {
    const grid = difficultyRef.current.grid;
    ...
    try {
      import('canvas-confetti').then((confetti) => {
        confetti.default({
          ...
          spread: 80,
          origin: { y: 0.6 },
          colors: ['#00f0ff', '#00f0ff', '#ffffff', '#10B981']
        });
      }).catch(() => { });
    } catch (_) { }
    ...
    if ((currentPhase === 'capture' || currentPhase === 'solve') && canvasRef.current) {
      ...
      if (canvasCtx) {
        ...
        if (results.multiHandLandmarks && (window as any).drawConnectors && (window as any).drawLandmarks) {
          for (const landmarks of results.multiHandLandmarks) {
            ...
            (window as any).drawLandmarks(canvasCtx, landmarks, {
              color: currentPhase === 'solve' ? '#00f0ff' : '#10B981',
        ...
        if (currentPhase === 'capture') {
          ...
          // Label indicators — flip context to counter the CSS scale-x-[-1] mirror
          canvasCtx.save();
          canvasCtx.translate(width, 0);
          canvasCtx.scale(-1, 1);
          const mirroredBoxX = width - (boxX + boxW);
          canvasCtx.fillStyle = '#00f0ff';
          canvasCtx.font = 'bold 14px monospace';
          canvasCtx.shadowBlur = 4;
          canvasCtx.shadowColor = '#00f0ff';
          canvasCtx.fillText('PINCH TO CAPTURE', mirroredBoxX, boxY - 10);
          canvasCtx.restore();
        }
  ...
  const renderCursor = () => {
    ...
    return (
      ...
      >
        <div className={`absolute inset-0 rounded-full border-2 border-[#00f0ff] transition-all duration-150 ${cursorPos.pinching ? 'bg-[#00f0ff]/40 scale-75' : 'bg-transparent scale-100'} shadow-[0_0_15px_rgba(0,240,255,0.5)]`} />
        {phase === 'solve' && draggedPiece && (
          <motion.div
            initial={{ scale: 0.8, opacity: 0 }}
            animate={{ scale: 1.15, opacity: 0.85 }}
            className="absolute rounded-xl overflow-hidden border-2 border-[#00f0ff] shadow-[0_0_25px_rgba(0,240,255,0.5)] pointer-events-none z-50 w-28 h-28"
            style={{
              left: 45,
              top: 45,
            }}
    ...
    <div className="flex flex-col items-center justify-center h-full space-y-12 animate-fade-in z-10 relative">
      <div className="text-center space-y-4">
        <h1 className="text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-zinc-400 tracking-tighter">
          PINCH <span className="text-[#00f0ff]">CRAFT</span>
        </h1>
      ...
      <div className="flex gap-4">
        {Object.values(DIFFICULTIES).map((diff) => (
          <button
            ...
            className={`px-6 py-3 rounded-xl border-2 font-bold tracking-wider transition-all duration-300 ${difficulty.id === diff.id
              ? 'border-[#00f0ff] text-black bg-[#00f0ff] shadow-[0_0_20px_rgba(0,240,255,0.4)] scale-105'
      ...
      <div className="flex gap-6">
        <button
          ...
          className="group relative px-8 py-4 bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden hover:border-[#00f0ff]/50 transition-colors"
        >
          <div className="absolute inset-0 bg-[#00f0ff]/10 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out" />
          <div className="relative flex items-center gap-3 text-white font-semibold">
            <Play size={20} className="text-[#00f0ff]" />
            START GAME
          </div>
        </button>
      ...
      <div className="bg-[#111] border border-zinc-800 rounded-3xl w-full flex flex-col p-8 shadow-2xl relative overflow-hidden backdrop-blur-xl">
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center gap-3">
            <List className="text-[#00f0ff]" size={28} />
            <h2 className="text-3xl font-black tracking-widest text-white">LEADERBOARD</h2>
          </div>
          <div className="flex items-center gap-2 border border-[#00f0ff]/30 bg-[#00f0ff]/5 rounded-full px-4 py-1.5 text-xs text-[#00f0ff] font-bold">
            <div className="w-2 h-2 rounded-full bg-[#00f0ff] animate-pulse" />
            LIVE
          </div>
        </div>

        <div className="grid grid-cols-6 text-xs font-bold tracking-widest text-zinc-500 pb-4 border-b border-zinc-800/50 mb-4 px-4 uppercase">
          <div>Rank</div>
        ...
        <div className="flex flex-col gap-2 overflow-y-auto max-h-[40vh] pr-2 custom-scrollbar">
          {leaderboard.filter(e => e.difficulty === difficulty.id).slice(0, 10).map((entry, idx) => (
            <div
              key={entry.id || idx}
              className={`grid grid-cols-6 items-center py-4 px-4 rounded-xl transition-colors ${entry.playerName.includes('YOU') ? 'bg-[#00f0ff]/10 border border-[#00f0ff]/30' : 'hover:bg-white/5 border border-transparent'}`}
            >
              <div className={`font-black text-lg ${idx < 3 ? 'text-[#00f0ff]' : 'text-zinc-600'}`}>
                #{idx + 1}
              </div>
              ...
              <div className="text-right text-[#00f0ff] font-mono font-bold text-lg">
                {formatTime(entry.completionTime)}
              </div>
              <div className="text-right flex justify-end">
                <button
                  onClick={() => {
                    if (confirm(`Delete stats for ${entry.playerName}?`)) {
        ...
        <div className="mt-8 flex justify-between items-center pt-6 border-t border-zinc-800/50">
          ...
          <button
            ...
            className="bg-[#00f0ff] text-black font-black uppercase tracking-wider py-3 px-8 rounded-xl hover:bg-white hover:shadow-[0_0_20px_rgba(255,255,255,0.4)] transition-all flex items-center gap-2 text-sm"
          >
            <RotateCcw size={16} />
            Back to Menu
          </button>
  ...
  return (
    <div className="relative w-full h-screen bg-zinc-950 font-sans overflow-hidden text-white selection:bg-[#00f0ff] selection:text-black flex items-center justify-center">
      {/* Background elements */}
      <div className="absolute inset-0 z-0 opacity-20 pointer-events-none">
        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-[#00f0ff] rounded-full blur-[150px] mix-blend-screen opacity-30 animate-pulse"></div>
      ...
      {!mpLoaded && !mpError && (
        <div className="absolute bottom-4 right-4 z-50 flex items-center gap-2 bg-zinc-900/80 border border-zinc-800 px-3 py-2 rounded-full backdrop-blur-sm pointer-events-none">
          <div className="w-3 h-3 border-2 border-zinc-700 border-t-[#00f0ff] rounded-full animate-spin" />
          <span className="text-[10px] text-zinc-500 font-mono tracking-wider">LOADING AI ENGINE...</span>
        </div>
      )}
      ...
      <div className={`absolute inset-0 z-10 flex flex-col items-center justify-center transition-opacity duration-500 ${phase === 'capture' || phase === 'solve' ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className="absolute top-6 w-full max-w-6xl px-6 flex justify-between items-start z-30 pointer-events-none">
          ...
          {phase === 'solve' && (
            <div className="flex flex-col items-center">
              <div className="flex items-center gap-2 bg-[#111]/80 backdrop-blur-md border border-zinc-800 px-6 py-2 rounded-full shadow-lg">
                <Clock size={16} className={isTimerRunning ? 'text-[#00f0ff] animate-pulse' : 'text-zinc-500'} />
          ...
          <div className="bg-[#111]/80 backdrop-blur-md border border-zinc-800 rounded-xl p-4 text-xs font-mono w-64 shadow-2xl">
            {phase === 'capture' ? (
              <>
                <div className="text-[#00f0ff] font-bold mb-2 tracking-widest flex items-center gap-2">
                ...
                <ol className="text-zinc-400 space-y-2 list-decimal pl-4">
                  ...
                  <li><span className="text-xs text-[#00f0ff] block mt-1">Or tap the "Capture Frame" button below to bypass gesture.</span></li>
                </ol>
              </>
            ) : (
              <>
          ...
          {phase === 'capture' && (
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-30 pointer-events-auto flex flex-col items-center gap-3">
              <button
                ...
                disabled={!isReady}
                className="px-6 py-3 rounded-xl bg-[#00f0ff] hover:bg-white text-black font-black uppercase text-xs tracking-wider transition-all disabled:opacity-50 flex items-center gap-2 shadow-[0_0_20px_rgba(0,240,255,0.4)]"
              >
                <Camera size={16} />
              ...
              <div className="flex flex-col items-center space-y-4">
                <div className="w-32 h-32 rounded-full border-4 border-zinc-800 flex items-center justify-center relative">
                  <svg className="absolute inset-0 transform -rotate-90 w-full h-full">
                    <circle cx="64" cy="64" r="60" stroke="#00f0ff" strokeWidth="8" fill="none" strokeDasharray="377" strokeDashoffset={377 - (377 * captureProgress) / 100} className="transition-all duration-100 ease-linear" />
                  </svg>
                  <Target className="text-[#00f0ff] animate-pulse" size={40} />
                </div>
                <p className="font-mono text-[#00f0ff] font-bold tracking-widest">HOLD SNAP...</p>
              </div>
            </div>
          )}
                ...
                {pieces.map((piece) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`absolute overflow-hidden group cursor-pointer border ${isDragged ? 'border-[#00f0ff] z-20 shadow-[0_0_20px_rgba(0,240,255,0.4)]' : 'border-transparent z-10'}`}
                      style={{
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3 bg-white/5 border border-white/10 rounded-xl p-4 mb-6 text-center">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xl font-black text-[#00f0ff] mt-0.5 uppercase">{difficulty.id}</span>
                </div>
              </div>
